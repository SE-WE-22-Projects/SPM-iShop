...
const getStoreMap = async (req, res) => {
  ...
  const storeSize = sectionData.reduce(
    (prev, current) => {
      return {
  ...
  const tags = tagData.map((t) => {
    ...
    if (
      t.pos_x > section.bottom_x ||
      t.pos_x < section.top_x ||
      t.pos_y > section.bottom_y ||
      t.pos_y < section.top_y
    ) {
...
const getAvailableIpAddress = async (req, res) => {
  ...
  const qr = await QRCode.toDataURL(
    JSON.stringify({ t: "SV", i: ipArray, ms: 0.17 }),
    { width: 750 }
  );
