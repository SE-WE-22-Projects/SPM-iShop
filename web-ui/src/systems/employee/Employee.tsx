...
import EmployeeTabel, {
  EmployeeData,
} from "./employeeComponents/EmployeeTabel";
import AddEmployeeeModel, {
  EmpAddData,
} from "./employeeComponents/AddEmployeeeModel";
...
const Employee = () => {
  ...
  const addEmployee = async (empData: EmpAddData) => {
    ...
    if (!empData.name) {
      ...
      enqueueSnackbar("Employee birth date is required...", {
        variant: "error",
      });
      return;
      ...
      !/^\d{9,12}$/.test(empData.contactNumber)
    ) {
      enqueueSnackbar("Employee contact number is required...", {
        variant: "error",
      });
      return;
      ...
      enqueueSnackbar("Employee hire date is required...", {
        variant: "error",
      });
      return;
    } else if (!empData.basicSalary) {
      enqueueSnackbar("Employee basic salary is required...", {
        variant: "error",
      });
      return;
    } else if (empData.basicSalary < 0) {
      enqueueSnackbar("Employee basic salary should be positive...", {
        variant: "error",
      });
      return;
    ...
    try {
      await axios.post("api/employee/employee", {
        ...empData,
        contactNumber: parseInt(empData.contactNumber),
      });
  ...
  const updateEmployee = (empData: EmployeeData) => {
    ...
    confirm({ description: "Confirm Employee Details" })
      .then(async () => {
        try {
          await axios.put(`api/employee/employee/${empData.id}`, {
            ...empData,
            contactNumber: parseInt(empData.contactNumber!!),
          });
          enqueueSnackbar("Employee updated successfuly...", {
            variant: "success",
          });
    ...
    confirm({ description: "Confirm Delete Employee Details" })
      .then(async () => {
        try {
          ...
          enqueueSnackbar("Employee deleted successfuly...", {
            variant: "success",
          });
          employeeUpdateModalClose();
          getEmployees();
      ...
      {isLoading ? (
        ...
        <div>
          ...
          <EmployeeTabel
            ...
            employeeUpdateModalOpen={employeeUpdateModalOpen}
            employeeUpdateModalClose={employeeUpdateModalClose}
            updateEmployee={updateEmployee}
            deleteEmployee={deleteEmployee}
          />
          <ReportGenerator
            children={<EmployeeAnalytics data={empList} />}
            filename="employee-report"
            title="Employee Details Report"
          />
          <AddEmployeeeModel
            employeeAddModalClose={employeeAddModalClose}
            open={addOpen}
